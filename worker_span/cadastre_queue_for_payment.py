...
from managers.rabbit_mq_manager import CadastreConsumerRabbitMQManager
...
class CadastreHandlerWorker:
    ...
    async def exec_scan(self):
        ...
        scan = True
        while scan:
            key_record = CadastreConsumerRabbitMQManager.read_message()
            # Берем последнее значение из очереди, удаляя его
            record = RedisManager.get_record(key=key_record)
            if record.calculate:
                pass
            # Если нет никакиъ
            else:
                result = billing_request(record=record)
                record.calculate.calculate = result
